...
export default function HeatmapTable() {
  ...
  const sorted = [...GENS].sort((a, b) => {
    const av = sortKey === 'generation' ? a.generation : a.childScores[sortKey] ?? 0;
    const bv = sortKey === 'generation' ? b.generation : b.childScores[sortKey] ?? 0;
          ...
          <tr style={{ borderBottom: '1px solid #1e293b' }}>
            ...
            >
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                GEN
                {sortKey === 'generation' && (sortDir === 'asc' ? <ChevronUp size={10} /> : <ChevronDown size={10} />)}
              </span>
            </th>
            {KEYS.map(k => (
              <th
            ...
            <th style={{ padding: '10px 12px', textAlign: 'center', fontFamily: 'JetBrains Mono', fontSize: 11, color: '#64748b' }}>
              STATUS
            </th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((g, i) => (
            <tr
              ...
              onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? 'transparent' : 'rgba(12,16,24,0.4)'}
            >
              <td style={{ padding: '8px 12px', fontFamily: 'JetBrains Mono', fontSize: 12, color: '#94a3b8' }}>
                G{g.generation}
              </td>
              {KEYS.map(k => {
                const val = g.childScores[k];
                return (
                  <td
                    key={k}
                    style={{ padding: '6px 6px', textAlign: 'center' }}
                    onMouseEnter={e => setHover({ x: e.clientX, y: e.clientY, label: BENCHMARK_LABELS[k], value: val })}
