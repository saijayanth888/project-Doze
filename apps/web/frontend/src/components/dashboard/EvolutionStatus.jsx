...
export default function EvolutionStatus() {
  ...
  return (
    <div className="mf-card-hover" style={{ background: C.bgC, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20, position: 'relative', overflow: 'hidden', height: '100%' }}>
      {isRunning && (
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {isRunning ? (
            <button type="button" onClick={handleStop} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', background: C.dangerDim, color: C.danger, border: `1px solid rgba(239,68,68,0.3)`, borderRadius: 4, cursor: 'pointer', fontFamily: F.ui, fontSize: 12, fontWeight: 600 }}>
            ...
            <button type="button" className="mf-cta-primary" onClick={handleStart} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', background: C.acc, color: '#000', border: 'none', borderRadius: 4, cursor: 'pointer', fontFamily: F.ui, fontSize: 12, fontWeight: 700, boxShadow: `0 0 12px ${C.accGlow}` }}>
