...
export default function ChampionCard() {
  ...
  const avgScore = champion?.avg_score || (champion?.scores
    ? Object.values(champion.scores).reduce((a, b) => a + b, 0) / Object.values(champion.scores).length
    : 0);

  return (
    <div style={{ background: C.bgC, border: `1px solid rgba(118,185,0,0.25)`, borderRadius: 8, padding: 18, height: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ paddingTop: 12, borderTop: `1px solid ${C.border}`, display: 'flex', gap: 6 }}>
        <button style={{ padding: '5px 12px', background: 'transparent', color: C.acc, border: `1px solid ${C.borderA}`, borderRadius: 4, cursor: 'pointer', fontFamily: F.ui, fontSize: 12, fontWeight: 500 }}>
          Run Inference
        </button>
