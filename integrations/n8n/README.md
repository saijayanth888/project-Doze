...
| `auto-cleanup.json` | Every 24h → `POST /api/adapters/cleanup` (`older_than_days=30`, `keep_promoted=5`). Optional Slack summary; Error Trigger forwards to `/api/system/alerts`. |
| `auto-retrain-on-upload.json` | Webhook `POST /webhook/dataset-uploaded`. Point **`N8N_WEBHOOK_DATASET_URL`** on the API at this URL. Waits/re-checks if evolution is running, then `POST /api/evolve/start` with `custom_dataset_id` from the payload. |
| `regression-guard.json` | Webhook `POST /webhook/evolution-complete`. Point **`N8N_WEBHOOK_EVOLUTION_COMPLETE_URL`** on the API at this URL. Compares last two rows from `/api/eval/generations`; optional `POST .../rollback` on regression + Slack. |
